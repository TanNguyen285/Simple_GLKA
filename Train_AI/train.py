...
from Simple_Anphax import Simple_GLKA as Simple_CNN


# ===== HELPER: Lấy lr, weight_decay, momentum an toàn từ config =====
def _get_cfg(key, fallback):
    """Lấy giá trị từ OPTIMIZER_CONFIG, nếu không có thì dùng fallback."""
    return config.OPTIMIZER_CONFIG.get(key, fallback)
...
def save_classification_report(all_preds, all_labels, class_names, save_dir,
...
def plot_tsne_features(all_features, all_labels, class_names, save_dir, epoch=None):
    try:
        ...
        title_epoch = epoch + 1 if epoch is not None else '?'
        plt.title(f't-SNE: Feature Separation (Epoch {title_epoch})',
...
def generate_gradcam_heatmap(model, val_loader, device, save_dir, epoch, class_names):
    try:
        ...
        plt.close()
        print(f"  -> Đã lưu Grad-CAM heatmap: {gradcam_path}")
...
def train_model():
    ...
    """
    # Tạo thư mục lưu kết quả TRƯỚC KHI dùng config.SAVE_DIR
    create_save_dir()
    ...
    # Lấy lr/weight_decay/momentum từ OPTIMIZER_CONFIG (an toàn với cả SGD & AdamW)
    LEARNING_RATE = _get_cfg('lr', 1e-3)
    WEIGHT_DECAY  = _get_cfg('weight_decay', 0.0)
    MOMENTUM      = _get_cfg('momentum', 0.0)   # AdamW không có momentum → 0.0
    ...
    train_loader, val_loader, class_to_idx = get_data_loaders(
        ...
        handle_imbalance=False   # Đổi True nếu dataset mất cân bằng
    )
    class_names = list(class_to_idx.keys())
    print(f"[*] Số lớp: {len(class_names)} | Classes: {class_names}")

    # ===== MÔ HÌNH =====
    model     = Simple_CNN(num_classes=config.NUM_CLASSES).to(DEVICE)
    ...
    if config.OPTIMIZER_TYPE == "SGD":
        ...
        raise ValueError(f"Unknown optimizer: {config.OPTIMIZER_TYPE}. Choose 'SGD' or 'AdamW'.")
...
# ========== TRAINING LOOP ==========
    for epoch in range(EPOCHS):
        ...
        print(f"\nResults: LR={current_lr:.6f} | Train Loss={train_loss:.4f} "
        ...
        if should_generate_gradcam:
            print(f"  [*] Đang sinh Heatmap tại epoch {epoch + 1}...")
